...
export class ExpenseRepository {
	...
	clear(): void {
		...
		this.db.run("DELETE FROM sqlite_sequence WHERE name = 'expenses'");
	}
	...
	list(query: ListExpensesQueryType): ExpenseListResponseType {
		...
		if (category) {
			...
			const catRow = this.db.query(catSumSql).get({ $category: category }) as {
				sum: number;
			};
			...
			const rows = this.db.query(subSql).all() as Array<{
				category: string;
				sum: number;
			}>;
			...
			for (const row of rows) {
				if (
					allCategories.includes(row.category as (typeof allCategories)[number])
				) {
					subtotals[row.category as keyof typeof subtotals] =
	...
	exportCsv(query: ListExpensesQueryType): string {
		...
		let sql =
			"SELECT id, amount, category, description, date, created_at FROM expenses";
		const params: Record<string, string> = {};
